...
#if defined(HITLS_CRYPTO_CODECSKEY) && defined(HITLS_CRYPTO_RSA) && defined(HITLS_CRYPTO_KEY_ENCODE)
#include "securec.h"
#include "bsl_sal.h"
...
typedef struct {
    ...
    BSL_Buffer *password;
} CRYPT_ENCODE_RSA_Ctx;

void *CRYPT_ENCODE_RSA_NewCtx(void *provCtx)
{
    CRYPT_ENCODE_RSA_Ctx *ctx = BSL_SAL_Calloc(1, sizeof(CRYPT_ENCODE_RSA_Ctx));
    if (ctx == NULL) {
...
int32_t CRYPT_ENCODE_RSA_SetParam(void *ctx, const BSL_Param *param)
{
    ...
    CRYPT_ENCODE_RSA_Ctx *rsaCtx = (CRYPT_ENCODE_RSA_Ctx *)ctx;
    
    for (const BSL_Param *p = param; p->key != 0; p++) {
        switch (p->key) {
            case CRYPT_PARAM_ENCODE_PROVIDER_CTX:
                rsaCtx->mgrCtx = (CRYPT_EAL_ProvMgrCtx *)p->value;
                break;
            case CRYPT_PARAM_ENCODE_PASSWORD:
                rsaCtx->password = (BSL_Buffer *)p->value;
                break;
            default:
                break;
        }
...
int32_t CRYPT_ENCODE_RSA_Encode(void *ctx, const BSL_Param *inParam, BSL_Param **outParam)
{
    ...
    for (const BSL_Param *p = inParam; p->key != 0; p++) {
        switch (p->key) {
            case CRYPT_PARAM_ENCODE_OBJECT_DATA:
                rsaCtx->pkey = (CRYPT_EAL_PkeyCtx *)p->value;
                break;
            default:
                break;
        }
    ...
    if (CRYPT_EAL_PkeyGetId(rsaCtx->pkey) != CRYPT_PKEY_RSA) {
    ...
    /* Encode RSA key to DER format */
    CRYPT_EncodeParam encodeParam = {0};
    CRYPT_Pbkdf2Param pbkdfParam = {0};
    if (rsaCtx->password != NULL) {
        pbkdfParam.pwd = rsaCtx->password->data;
        pbkdfParam.pwdLen = rsaCtx->password->dataLen;
    ...
    rsaCtx->outputType = (keyType == CRYPT_PRIKEY_PKCS8_UNENCRYPT) ? "PKCS8" : "SubjectPublicKeyInfo";
...
#endif /* HITLS_CRYPTO_CODECSKEY && HITLS_CRYPTO_RSA && HITLS_CRYPTO_KEY_ENCODE */